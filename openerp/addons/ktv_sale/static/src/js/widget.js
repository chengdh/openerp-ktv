    ...
    widget.BaseRoomCheckoutWidget = erp_instance.web.OldWidget.extend({
        ...
        _autoset_pay_type_member_card_fee : function(){
            if(this.member.get("id"))
            {
                ...
                //如果余额为0,则不支付
                if(balance == 0.0)
                    this.model.set({'member_card_fee' : 0.0 });
                else if(balance >= cash_fee + member_card_fee)
                    this.model.set({'member_card_fee' : cash_fee + member_card_fee});
                else
                    this.model.set({'member_card_fee' : balance});
            }
        },
        //重新显示会员信息
        render_member_card_no: function() {
        ...
        _autoset_pay_type_credit_card_fee : function(){
            if (this.credit_card.get("card_no")) {
                //默认设置信用卡支付金额为全款
                this.model.set({'credit_card_fee' : this.model.get('cash_fee')});
            }
            else
                this.model.set({'credit_card_fee' : 0.0});
        },
        //显示信用卡信息
        ...
        _onchange_member_card_fee : function(){
            ...
            if(member_card_fee > this.member.get('balance')){
                ...
                this.model.set({'member_card_fee' : origin_member_card_fee});
